...
function parseExpenseMessage(message) {
  ...
  const amountMatch = message.match(/(?:\$)?\s?([\d,]+(?:\.\d{1,2})?)\s?(?:dollars)?/i);
  ...
  let item = null;
  const patterns = [
    /(?:bought|purchased|got|spent on|paid for)\s+(?:\d+\s*(?:dollars)?\s*)?(?:worth of\s+)?([\w\d\s-]+?)(?=\s(?:at|from|\$|\d|today|yesterday|on))/i,
